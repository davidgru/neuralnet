...
{
    uint32_t status = dataset_create(out_train_set, impl, train_config, normalize, NULL);
    if (status != 0) {
        return status;
    }

    const dataset_statistics_t* train_statistics = dataset_get_statistics(*out_train_set);
    status = dataset_create(out_test_set, impl, test_config, normalize, train_statistics);
    if (status != 0) {
