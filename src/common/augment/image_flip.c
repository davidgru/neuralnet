...
static void aug_flip_inplace_priv(augment_context_t* context, tensor_t* input_output);
...
const augment_impl_t aug_image_flip = {
    ...
    .deinit_func = NULL,
    .augment_func = NULL,
    .augment_inplace_func = aug_flip_inplace_priv,
...
)
{
    ...
    for (size_t ch = 0; ch < channels; ch++) {
        float* current_image = &image[ch * channel_size];
...
static void aug_flip_inplace_priv(augment_context_t* context, tensor_t* input_output)
{
    ...
    for (size_t n = 0; n < batch_size; n++) {
        ...
        flip_image_inplace(&data[n * per_batch_size], channels, height, width, flip_horizontal, flip_vertical);
    }
}
