...
uint32_t model_desc_create(model_desc_t** desc, input_dims_t* input_shape, int loss);

uint32_t model_desc_dump(model_desc_t* desc);

uint32_t model_desc_add_activation_layer(model_desc_t* desc, int kind);
uint32_t model_desc_add_linear_layer(model_desc_t* desc, float learning_rate, size_t OC, int weight_init, dnnl_linear_layer_bias_init_kind_t bias_init);
