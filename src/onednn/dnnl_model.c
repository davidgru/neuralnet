...
static uint32_t dnnl_model_forward(dnnl_model_t* model, float* input)
{
    CHECK_DNNL(dnnl_layer_set_input_handle(model->layers[0], input));
    for (size_t i = 0; i < model->num_layers; i++)
        CHECK_DNNL(dnnl_layer_fwd(model->layers[i]));
    return 0;
dnnl_error:
    return 1;
}
...
uint32_t dnnl_model_create(dnnl_model_t** model, input_dims_t* input_dims, size_t num_layers, dnnl_layer_create_info_t* layer_create_infos, dnnl_loss_kind_t loss_kind)
{
    ...
    *model = (dnnl_model_t*)malloc(sizeof(dnnl_model_t));

    dnnl_model_t* m = *model;

    m->layers = (dnnl_layer_t**)malloc(num_layers * sizeof(dnnl_layer_t**));
    m->num_layers = num_layers;
    m->physical_input_size = input_dims->C * input_dims->H * input_dims->W;

    // Create the input layer
    dnnl_input_layer_create_info_t i_info;
    ...
    dnnl_layer_create_info_t input_ci;
    input_ci.layer_kind = dnnl_layer_kind_input;
    input_ci.layer_create_info = &i_info;
    CHECK_DNNL(dnnl_layer_create(&m->input_layer, &input_ci));

    // Create all model layers
    for (size_t i = 0; i < m->num_layers; i++)
        CHECK_DNNL(dnnl_layer_create(&m->layers[i], &layer_create_infos[i]));

    // Create loss
    CHECK_DNNL(dnnl_loss_create(&m->loss, loss_kind));

    // Init fwd pass for all layers
    CHECK_DNNL(dnnl_layer_fwd_pass_init(m->input_layer, 0));
    CHECK_DNNL(dnnl_layer_fwd_pass_init(m->layers[0], m->input_layer));
    for (size_t i = 1; i < m->num_layers; i++)
        CHECK_DNNL(dnnl_layer_fwd_pass_init(m->layers[i], m->layers[i-1]));

    // Init loss
    CHECK_DNNL(dnnl_loss_init(m->loss, m->layers[m->num_layers-1]));
    
    // Init bwd pass for all layers
    CHECK_DNNL(dnnl_layer_bwd_pass_init_loss(m->layers[m->num_layers-1], (_dnnl_loss_t*)m->loss));
    for (size_t i = m->num_layers-2; i >= 1; i--)
        CHECK_DNNL(dnnl_layer_bwd_pass_init(m->layers[i], m->layers[i+1]));
    CHECK_DNNL(dnnl_layer_bwd_pass_init(m->input_layer, m->layers[0]));

    dnnl_layer_t* last_layer = m->layers[m->num_layers - 1];
    m->physical_output_size = last_layer->IC * last_layer->IH * last_layer->IW;


    memcpy(&m->input_shape, input_dims, sizeof(input_dims_t));
...
{
    return dnnl_model_create(model, &desc->input_shape, desc->num_layers, desc->create_infos, desc->loss);
}
...
{
    dnnl_training_progress_t progress_info;
    ...
    for (size_t i = 0; i < test_set_size; i += model->input_shape.N) {
        ...
        CHECK_DNNL(dnnl_model_forward(model, input));
        test_acc += dnnl_loss_acc(model->loss, labels);
        test_loss += dnnl_loss_loss(model->loss, labels);
    }
    ...
    for (uint32_t i = 0; i < num_epochs; i++) {
        ...
        for (size_t j = 0; j < train_set_size; j += model->input_shape.N) {
            ...
            CHECK_DNNL(dnnl_model_forward(model, input));
            // Evalutation
            train_acc += dnnl_loss_acc(model->loss, labels);
            train_loss += dnnl_loss_loss(model->loss, labels);
            // Backward pass
            CHECK_DNNL(dnnl_loss_bwd(model->loss, labels));
            for (size_t k = model->num_layers - 1; k >= 1; k--)
                CHECK_DNNL(dnnl_layer_bwd(model->layers[k]));
        }
...
{
    dnnl_loss_destroy(model->loss);
    for (size_t i = 0; i < model->num_layers; i++)
        dnnl_layer_destroy(model->layers[i]);
    dnnl_layer_destroy(model->input_layer);
