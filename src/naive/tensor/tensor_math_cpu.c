    ...
    size_t outer_stride, size_t outer_len, size_t axis_len, size_t inner_stride)
{
    for (size_t i = 0; i < outer_len; i++) {
        for (int k = 0; k < inner_stride; k++) {
            v->data[i * inner_stride + k] = 0;
            for (int j = 0; j < axis_len; j++) {
                float f = w->data[i * outer_stride + j * inner_stride + k]
                          - mean->data[i * inner_stride + k];
                v->data[i * inner_stride + k] += f * f;
            }
            v->data[i] /= axis_len;
        }
    }
...
void tensor_momentum_update_cpu(tensor_t* v, const tensor_t* w, float momentum)
{
    ...
    const float* w_data = tensor_get_data_const(w);
    size_t n = tensor_get_size(v);
    VectorScale(v->data, momentum, n);
    VectorScaledAdd(v->data, w->data, 1.0f - momentum, n);
}
