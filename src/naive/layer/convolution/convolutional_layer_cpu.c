    ...
    tensor_t* d_bias, int32_t stride_y, int32_t stride_x, int32_t padding_y, int32_t padding_x)
{
    // General formula:
    // dw = conv2d(x, dy,
    //        padding: (fw_padding_y, fw_padding_x),
    //        stride: (1, 1)
    ...
    //      dy: NxOCxOHxOW => OCxNxOHxOW
    //
    // 2. re-label dimensions (logical <-> input in conv):
    //      N <-> IC, OH <-> KH, OW <-> KW
    //       x: ICxNxIHxIW => NxICxIHxIW
    //      dy: OCxNxOHxOW => OCxICxKHxKW
    //
    // 3. conv(x, dy, dw, ...)
    //
    // 4. permute dw
    //      dw: NxOCxOHxOW
    //       => ICxOCxKHxKW (re-label)
    //       => OCxICxKHxKW (permute)
    ...
    // conv assumes pre-zeroed output
    memset(dw_p, 0, OC * IC * KH * KW * sizeof(float));

    // conv with permuted re-labeled tensors
    conv(x_p, dy_p, dw_p, IC, N, IH, IW,
         OC, OH, OW, padding_y, padding_x,
         1, 1, stride_y, stride_x, 0, 0);

    // permute back dw
    swap_N_C(dw_p, d_weights->data, OC, IC, KH, KW);
